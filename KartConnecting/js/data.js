...
function renderJugadores() {
  ...
  jugadores.forEach(p => {
    ...
    col.innerHTML = `
      <div class="item-card">
        <h3>${p.nombre}</h3>
        <p><strong>Pais:</strong> ${p.pais} • <strong>Nivel:</strong> ${p.nivel || p.level || 'N/A'}</p>
        <p>${p.bio}</p>
      </div>
    `;
...
function renderTorneos() {
  ...
  torneos.forEach(t => {
    ...
    item.innerHTML = `
      <h3>${t.nombre}</h3>
      <p><strong>Modalidad:</strong> ${t.modalidad} • <strong>Fechas:</strong> ${t.inicio} → ${t.fin}</p>
    `;
    // Añadimos link para ver carreras de ese torneo
    const carrerasDelTorneo = carreras.filter(c => c.id_torneo === t.id);
    if (carrerasDelTorneo.length) {
      const ul = document.createElement('ul');
      ul.style.textAlign = 'left';
      carrerasDelTorneo.forEach(c => {
        const li = document.createElement('li');
        li.textContent = `${c.pista} — ${c.fecha}`;
        ul.appendChild(li);
      });
      item.appendChild(ul);
    }
...
/* Función que muestra una sección y marca el link activo */
...
/* Al cargar la página, renderizamos las secciones */
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('lista-jugadores')) {
    renderJugadores();
  }
  if (document.getElementById('lista-equipos')) {
    renderEquipos();
  }
  if (document.getElementById('lista-torneos')) {
    renderTorneos();
  }
});

  // Asegurar que inicio está visible
